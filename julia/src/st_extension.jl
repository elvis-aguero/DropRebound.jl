...
Fill residual R (length 3M+1) with Carreau shear-thinning correction to R2.

Calls build_residual! for the Newtonian part, then corrects the R2 block:
  R2[k] -= dt * D2[k] * Ȧ_curr[k] * ε_ST * shear_sq_lag

where shear_sq_lag = Σ_k Gamma_eff[k] * Ȧ_prev[k]² is lagged (history[end]).
...
function build_residual_st!(R::AbstractVector, state::DropState,
    ...
    Gamma_eff = st.Gamma .* (st.lambda_c .* sigma0) .^ 2

    Adot_prev    = history[end].Adot[2:end]
    shear_sq_lag = sum(Gamma_eff[k] * Adot_prev[k]^2 for k in 1:M-1)

    Adot_curr = state.Adot[2:end]
    R[M:2M-2] .-= dt .* D2 .* Adot_curr .* (st.eps_ST * shear_sq_lag)
end

"""
    build_jacobian_st(state, history, dt, cp, cfg, ob, st) → Matrix{Float64}

Analytical Jacobian for Carreau system. Identical to build_jacobian except
the ∂R2/∂Ȧ diagonal is multiplied by (1 - ε_ST * shear_sq_lag).
shear_sq_lag is lagged (uses history[end].Adot), so the Jacobian is constant
across Newton iterations within a step.
"""
function build_jacobian_st(state::DropState, history::Vector{DropState},
    ...
    for k in 1:Nm
        J[Nm+k, Nm+k] -= dt * D2[k] * st.eps_ST * shear_sq_lag
    end
    return J
end
