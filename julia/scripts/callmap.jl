...
const SRC  = joinpath(ROOT, "julia", "src")
const USERS = [joinpath(ROOT, "julia", "test"), joinpath(ROOT, "julia", "scripts")]
...
open(joinpath(ROOT, "results", "callmap.csv"), "w") do io
    println(io, "from,to")
    for f in files, t in sort(collect(edges[f])); println(io, "$f,$t"); end
end
open(joinpath(ROOT, "results", "callmap_nodes.csv"), "w") do io
...
flushln("\nwrote results/callmap.csv and results/callmap_nodes.csv")
