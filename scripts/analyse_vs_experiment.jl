...
const STORE = joinpath(ROOT, "outputs", "csv", "runs.csv")
const MIN_REPS = 5
...
println("="^104)
chdr, crows = read_csv(joinpath(ROOT, "data","gabbard2025_restitution.csv"))
thdr, trows = read_csv(joinpath(ROOT, "data","gabbard2025_contact_time.csv"))
...
for (i,ln) in enumerate(eachline(joinpath(ROOT, "derivations","data","metrics_3000ppm.csv")))
    i == 1 && continue
