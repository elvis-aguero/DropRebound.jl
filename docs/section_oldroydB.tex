...
Every result in Sections 1--9 rests on a single constitutive assumption:
the viscous stress is instantaneously proportional to the rate of strain,
...
this memory faithfully while remaining analytically tractable. It is the
...
The central result of this section is that Reid's characteristic equation
\eqref{eq:char_eq} survives the generalisation to Oldroyd-B: the left-hand
side $\alpha^4/q^4 + 1$ is unchanged, and the right-hand side is Reid's
right-hand side with $q$ replaced by a modified wavenumber $q_*$ that is a
rational function of $q^2$. This rational structure introduces a new type
of pole --- the \emph{retardation pole} --- which captures additional
eigenvalues beyond Reid's fundamental pair and which is the mechanism by
which the two-number $\{A_l, D_l\}$ picture eventually breaks down.
...
\end{notebox} When $De_1 \ll 1$: polymer relaxes many times
per cycle, the drop behaves as Newtonian. When $De_1 \sim 1$: elasticity
...
It is not itself a root of the characteristic equation $F(w) = 0$; rather,
the actual retardation roots of the characteristic equation lie in a
neighbourhood of $w_{\mathrm{ret}}$ (see Section~\ref{sec:OB_roots}).
...
in the effective wavenumber that has no analogue in the Maxwell or
Newtonian problems. All qualitative differences between Oldroyd-B and
Maxwell trace back to this pole.
\end{notebox}
...
is always to the right of the relaxation zero on the positive real $q^2$-axis.
\end{notebox}
...
Following the derivation of Section~5 verbatim, with $\nu \to \nu_{\mathrm{eff}}$
and hence $q^2 \to q_*^2$ throughout:
...
the energy. This has no Newtonian analogue --- it is intrinsically a
two-component phenomenon. Physically, it corresponds to the elastic recoil
...
This is a \emph{structured approximation}: the convolution kernel replaces the
full Newtonian damping function $D_l(\mathrm{Oh})$ by the zeroth moment
$\tilde{K}^{(\mathrm{cap})}(0)=1$, capturing the frequency-dependent viscosity
$\nu_{\mathrm{eff}}(\beta\sigma_{l;0})/\nu$ but not the Oh-dependence encoded in
...
\begin{enumerate}
  ...
  \item \textbf{Experimental validation.} To the authors' knowledge, no
