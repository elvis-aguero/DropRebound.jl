...
This sounds deceptively simple, but it couples three distinct pieces of physics:
\begin{enumerate}
...
the problem to a single transcendental characteristic equation. Its central insight is
an elegant rescaling that shows the problem is \emph{universal}: the physics of surface
tension enters only through the inviscid frequency $\sigma_{l;0}$, and after a suitable
non-dimensionalization, the characteristic equation is \emph{identical} to the one derived
by Chandrasekhar for a self-gravitating viscous liquid globe — a seemingly unrelated
problem.
...
\begin{notebox}
A crucial connection: any function of the form $U(x) = x\,j_l(qx)$ satisfies the ODE
\begin{equation}
...
The scaling $1/x^2$ is conventional and simplifies the algebra significantly.
It arises from the stream-function representation of poloidal flow, and ensures
the ODE for $U$ takes the clean form \eqref{eq:ODE_U} below.
\end{notebox}

\subsection{Derivation of the ODE for $U(x)$}

\begin{derivbox}[Full derivation]
We substitute the ansatz $u_r = \sigma R\,G(x)\,Y_l^m$ (where $G = U/x^2$, absorbing
the $\epsilon_0 e^{-\sigma t}$ factors throughout) into the $r$-component of
...
one can show (after substantial but straightforward algebra) that $\tau_{r\theta} = 0$ at
$x=1$ reduces to:
\begin{equation}
...
This is the most physically rich boundary condition, as it involves surface tension
and couples the pressure, viscous stress, and surface curvature.
...
this is not a coincidence: both arise from the curvature response to a harmonic
deformation.
\end{derivbox}
...
\begin{equation}
  ...
  \label{eq:alpha4}
\end{equation}
Crucially, $T_1$ and $\rho$ have \emph{completely cancelled}. The problem in terms
...
Thus \eqref{eq:char_eq} is universal for any spherically symmetric restoring force.
\end{notebox}
...
The algebra is lengthy but elementary; each step uses only linear algebra and
...
\begin{equation}
  ...
  \label{eq:lamb}
\end{equation}
recovering \emph{Lamb's classical result}: viscosity simply adds a real damping rate
...
representable. Higher radial overtones involve internal nodal surfaces that have no
counterpart in the quasi-static shape family and decay on timescales $\ll (R^2/\nu)$,
well separated from the impact timescale.
