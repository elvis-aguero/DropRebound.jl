...
#
# `src/st_extension.jl` builds its shear-thinning damping correction by
...
# ``\varepsilon_{ST}=(1-n)/a`` per `src/types.jl`'s `STParams`
# docstring.
#
...
# already multiplying the damping coefficient in `src/st_extension.jl`,
