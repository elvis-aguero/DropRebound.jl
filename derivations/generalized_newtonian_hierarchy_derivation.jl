...
# | ``\zeta_l(t)`` | amplitude of surface mode ``l``; the boundary trace of the state, not the state itself |
...
# ```
#
# the surface of the drop is described by the modal amplitudes ``\{\zeta_l(t)\}``,
# ``l=2\ldots M``. This is exactly the state `src/types.jl` carries.
#
...
# ```
#
# applied to the single factor of ``(1-\mu^2)`` turns it into a combination of ordinary
# products. The sharp statement, though, is a degree count, and it is worth making that way
# because the shifted-product route only bounds ``k`` by ``l+m+2``, which is too weak for
# the truncation argument that depends on it. Since ``(1-\mu^2)P_l'`` has degree ``l+1``
# and ``P_m'`` has degree ``m-1``,
#
# ```math
# (1-\mu^2)\,P_l'(\mu)\,P_m'(\mu)\ \text{ is a polynomial of degree exactly } l+m ,
# ```
#
# so its integral against ``P_k`` vanishes for every ``k>l+m``. Parity follows separately
# from ``P_l(-\mu)=(-1)^lP_l(\mu)``: unless ``l+m+k`` is even the integrand is odd and the
# integral over a symmetric interval vanishes. So both families obey the same rule, and the
...
# the field. The companion page carries a decomposition of the same physics into families it
# calls ``G^k_{lm}`` and ``H^k_{lm}``; those are the **traction-route** angular integrals,
# reached by projecting the normal-stress balance rather than by differentiating the
# dissipation, and they are different integrals of the same three indices, obeying the same
# rule.
#
...
# does, exactly. Writing ``M_{ab}=\partial^2T/\partial\dot\xi_a\partial\dot\xi_b=
# \int\bm u^{(a)}\!\cdot\!\bm u^{(b)}dV`` and ``K_{ab}`` for the damping matrix boxed
# above, both restricted to the single radial function ``\psi_l\propto x^{l+1}`` so that
# each is one number per mode, and ``\lambda_l^{\text{Lamb}}=(l-1)(2l+1)\mathrm{Oh}`` for
# Lamb's classical result,
#
# ```math
# \frac{K_{ll}}{2\,M_{ll}}
#   \;=\;(l-1)(2l+1)\,\mathrm{Oh}
...
# \qquad
# r_\infty \equiv \frac{\eta_\infty}{\eta_0}\ \text{(the solver's \texttt{eta\_inf\_ratio})},
# ```
#
...
#
# **The shear rate is a field, not a per-mode number.** Strain rate is linear in the
# velocity, and the velocity is linear in the coordinate rates, so the strain **tensor**
# superposes over every degree of freedom:
#
# ```math
# \bm e(x,\theta,t) \;=\; \sum_{l=2}^{M}\sum_{k=1}^{K}\dot a_{l,k}(t)\;
#   \bm e^{(l,k)}(x,\theta) \;\equiv\; \sum_{a}\dot\xi_a\,\bm e^{(a)} .
# ```
#
# The sum runs over the full coordinate set ``a = (l,k)``, not over surface modes alone: the
# strain is not slaved to ``\dot{\bm\zeta}``, and writing it that way would be the ``K=1``
# special case. The invariant does not superpose:
#
# ```math
# \dot\gamma \;=\; \sqrt{2\,\bm e\!:\!\bm e}
#   \;=\; \Bigl(2\sum_{a,b}\dot\xi_a\dot\xi_{b}\;
#          \bm e^{(a)}\!:\!\bm e^{(b)}\Bigr)^{1/2}.
# ```
#
# It is quadratic in the field and *then* square-rooted, so the ``a\neq b``
# cross terms do not drop out. Once more than one degree of freedom is active there is no
...
# If contact occupies a single connected patch about the pole, the set of nodes in
...
# fluids satisfying (H1)--(H3) of *Hypotheses on the viscosity*:
